...
def generate_measurement_positions(start=(2, 5), end=(28, 5), interval=0.8):
    """
    生成直线路径上的测量位置
    ...
    for i in range(num_points):
        t = min(i * interval, distance)  # 确保不超过终点
        position = np.array(start) + t * unit_direction
...
class ProgressiveMapping(MultiPositionMapping):
    """
    继承MultiPositionMapping类，添加渐进式测量和可视化功能
    """
    ...
    def save_current_state(self, positions_so_far, step_number, real_map):
        ...
        # 保存概率地图
        prob_map_path = fr"C:\Users\11967\cursor_files\simulation_of_robotic_map\output\linear_probability_maps_step_{step_number:03d}.png"
        ...
        # 保存符号化地图
        symbolic_map_path = fr"C:\Users\11967\cursor_files\simulation_of_robotic_map\output\linear_reconstruction\linear_symbolic_map_step_{step_number:03d}.png"
...
def main():
    ...
    # 生成测量位置
    measurement_positions = generate_measurement_positions()
    print(f"Generated {len(measurement_positions)} measurement positions")
    
    # 创建渐进式建图实例，使用相同的分辨率
    mapper = ProgressiveMapping(grid_resolution=resolution, real_map=room_map)
